...
DNS Latency Test
Latency test famous DNS providers. Inspired by DNS-XS

DNS Hunter
Test to check what Iranian datacenters resolve dns requests to specific domains. Inspired by DNS Hunter

Edge IP Checker
Scan for accessible Cloudflare Edge IPs. Inspired by CF Scan Null VoiD
...
Replace the IP address in a vless config with other IPs from a list or IP ranges and get all the vless configs back. Inspired by v2ray config modifier

CDN Xray Scanner
Scan CDN IP ranges using Xray core and your own json config. Inspired by the GOAT, Null VoiD Scanner.

In order to prepare your json configuration for scanning read this guide.
...
SNI Spoof Check
Check if SNI spoofing is open. Inspired by sni scanner
...
تست تأخیر DNS
تست تأخیر ارائه‌دهندگان معروف DNS. الهام گرفته از DNS-XS

DNS Hunter
تست برای بررسی اینکه دیتاسنترهای ایرانی درخواست‌های DNS چه دامنه‌هایی را resolve می‌کنند. الهام گرفته از DNS Hunter

بررسی‌کننده Edge IP
اسکن برای یافتن IP های Edge کلودفلر که در دسترس هستند. الهام گرفته از CF Scan Tolid Melli
...
جایگزینی آدرس IP در کانفیگ vless با IP های دیگر از یک لیست یا بازه IP و دریافت تمام کانفیگ‌های vless. الهام گرفته از v2ray config modifier

اسکنر CDN Xray
اسکن بازه‌های IP سی‌دی‌ان با استفاده از هسته Xray و کانفیگ json شخصی شما. الهام گرفته از استاد، Null_void
برای گرفتن json از کانفیگ خود برای اسکن اینجا را مطالعه کنید.
ℹ️ توضیحات تکمیلی:
...
چک کردن sni spoof
چک کردن باز بودن sni spoof. الهام گرفته شده از sni scanner
...
FOR ANY QUESTION OR NEED SUPPORT TEXT ME ON TG @Nu11_VoiD